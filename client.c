...
int main(int argc, char **argv)
{
  ...
  struct msg messageForDatabase;
  int res;

  while (1)
  {

    res = read(socket_fd, &messageForDatabase, sizeof(struct msg));
    if (res == 0)
    {
    ...
    int type = messageForDatabase.type;
    ...
    case 1:
      ...
      break;
    case 2:
      printf("name: %s\nid %d", messageForDatabase.rd.name, messageForDatabase.rd.id);
      break;
    case 3:
      fprintf(stderr, "SHOULDNT OF GOT HERE");
    }
    break;
  }
  ...
  while (1)
  {
    ...
    messageForDatabase.rd = tempRecord;
    messageForDatabase.type = type;

    int wres = write(socket_fd, &messageForDatabase, sizeof(struct msg));
