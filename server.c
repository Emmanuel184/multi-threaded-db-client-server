...
void get(int32_t fd)
{
  ...
  if ((lseek(fd, sizeof(sr) * index, 0) >= lseek(fd, 0, SEEK_END)) || index < 0)
  {
    fprintf(stderr, "The given index is out of bounds\n");
    return;
  }

  lseek(fd, sizeof(sr) * index, SEEK_SET);

  read(fd, &s, sizeof(sr));

  if (s.name[0] == NULL)
  {
    fprintf(stderr, "No student record exists at the given index");
